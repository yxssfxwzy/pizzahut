...
function addRestaurant($restaurant) {
  ...
  while ($result!=true) {
    $bid=$result[0]+1;
    $query = "select * from branch where bid={$bid};";
    $result = mysql_query($query);
  ...
  if (isset($restaurant)) {
    $query = "insert into branch (bid,name, tel, address, coordinate) values ('{$bid}','".$restaurant->get_name()."','".$restaurant->get_tel()."','".$restaurant->get_address()."','".$restaurant->get_coordinate()."');";
    echo $query;
  } else {
    echo "The user is empty!";
    return false;
...
function editRestaurant($restaurant) {
  ...
  if (isset($restaurant)) {
    ...
    $query = "update branch set name='".$restaurant->get_name()."',  coordinate='".$restaurant->get_coordinate()."', address='".$restaurant->get_address()."',tel='".$restaurant->get_tel()."' where bid=".$restaurant->get_bid().";";
//    var_dump( $query);
//    exit();
  }
...
function toggleRestaurant($restaurant) {
  ...
  if (isset($restaurant)) {
    $query = "SELECT id, isActive from branch where id=".$restaurant->get_id().";";
  }
  ...
  if (isset($isNewActive)) {
    $query2 = "update branch set isActive='".$isNewActive."' where id=".$restaurant->get_id().";";
    $result2 = mysql_query($query2);
    if ($result2==true) {
      closeDB($connection);
      return true;
...
function deliverOrder($oid) {
  ...
  if (isset($oid)) {
    $query = "UPDATE `order` SET status = 'delivering' where oid=".$oid.";";
//    echo $query;
//    exit;
  } else {
    echo "Nothing to deliver!";
    return false;
  }
